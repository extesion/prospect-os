...
class YouTubeParser {
  ...
  parseCurrentChannelPage() {
    ...
    // Tenta encontrar ID no canonical link
    let channelId = null;
    ...
    // Se ainda não tiver channelId UC..., cria fallback determinístico baseado no handle
    if (!channelId && handle) {
      channelId = `HANDLE_${handle.replace("@", "")}`;
    }
  ...
  extractDataFromContainer(containerObj) {
    ...
    // Fallback: se não tiver UC... no href mas tiver @handle
    if (!channelId && handle) {
